...
(async () => {
  try {
    ...
    const normalized = {
      ...
      shiprocket_status: tracking?.tracking_data?.shipment_status?.current_status || tracking?.tracking_data?.shipment_status || tracking?.current_status || null,
    ...
    if (doUpdate) {
      ...
      if (normalized.track_url) updates.shiprocket_track_url = normalized.track_url;
